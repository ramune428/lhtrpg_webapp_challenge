...
└── backend/    # FastAPI 実装。現状の公開フロントエンドでは主処理としては使用していません。
```

## 現在の実装方針

現状の公開Webアプリでは、主な処理は `frontend` 側で実行しています。

キャラクター駒作成処理は、フロントエンドからログ・ホライズンTRPG冒険者窓口のJSONを取得し、ブラウザ上でCCFOLIA用の出力形式に変換します。

`backend` と `frontend/app/api/create-piece/route.ts` は、バックエンド経由で処理する構成を検討した際の実装です。今後バックエンド構成に寄せる場合は、READMEと起動手順を更新してください。
...
- `backend` は現状の公開フロントエンドの主処理では使用していません。バックエンド経由に切り替える場合は、APIルートと環境変数 `API_BASE_URL` の扱いを整理してください。
