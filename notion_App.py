import streamlit as st  # ターミナル -> streamlit run <file>
import requests
...
def create_piece(character_id):
    json_url = r"https://lhrpg.com/lhz/api/" + character_id + ".json"
    ...
    try:
        ...
        # キャラクター：名前/行動値/URL
        character_data, character_name = create_character_data(json_data,
                                                               character_id)

        # ステータス：HP/障壁/疲労/ヘイト/因果力/移動力
        status_data = create_status_data(json_data)

        # パラメータ：CR/攻撃力/魔力/回復力/物防/魔防 <-"value"にしないと反映されない
        # 各能力値（STR, DEX...追加）
        ...
        # チャットパレット
        chat_palette = create_chat_palette(skill_data, equipment_data,
...
with st.form(key="character_form"):
    ...
    # 1. ログ・ホライズン冒険者窓口のキャラクターID（キャラクター詳細）を入力
    character_url = st.text_input(
        "キャラクターURL")
    # character_id = "207538" #デバック用
    # character_url = 'https://lhrpg.com/lhz/pc?id=130743' #デバック用
    if type(character_url) is int:
        character_id = character_url
    else:
        character_url_split = character_url.rsplit('=')
        character_id = character_url_split[-1]

    # 実行ボタン
    action_btn_1 = st.form_submit_button("コマンドを生成する")

    if action_btn_1:
        piece = create_piece(character_id)
