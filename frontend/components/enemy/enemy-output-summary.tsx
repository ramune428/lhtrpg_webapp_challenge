import { getCombinedTagText, type EnemyFormData, type EnemySkillInput, type EnemyDropItemInput } from "@/utils/createEnemyPiece";

type EnemySkillOutputRow = EnemySkillInput & { id: string };
type EnemyDropItemOutputRow = EnemyDropItemInput & { id: string };

type EnemyOutputSummaryProps = {
  form: EnemyFormData;
  items: EnemyDropItemOutputRow[];
  outputSkills: EnemySkillOutputRow[];
};

export default function EnemyOutputSummary({
  form,
  items,
  outputSkills,
}: EnemyOutputSummaryProps) {
  const visibleItems = items.filter(
    (item) => item.name.trim() || item.dice.trim(),
  );
  const visibleSkills = outputSkills.filter(
